...
def create_Employee(quickbooks_obj):
	...
	## FIXED FOR USA QB Account
	try:
		count = 1
		for employee in get_qb_employee['Employee']:
			if employee['Id']:
				if not frappe.db.get_value("Employee", {"quickbooks_emp_id": str(employee['Id'])}, "name"):
					employee_doc = frappe.new_doc("Employee")
					employee_doc.employee_name = str(employee.get('DisplayName'))
					employee_doc.quickbooks_emp_id = str(employee.get('Id'))
					employee_doc.gender = employee.get('Gender') if employee.get('Gender') else "Male"
					employee_doc.date_of_joining = employee.get('HiredDate') if employee.get('HiredDate') else strftime("%Y-%m-%d")
					employee_doc.date_of_birth = employee.get('BirthDate') if employee.get('BirthDate')  else "2013-04-01"
					employee_doc.flags.ignore_permissions = 1
					employee_doc.flags.ignore_mandatory = 1
					employee_doc.save()
					count = count +1
					quickbooks_employee_list.append(str(employee.get('Id')))
